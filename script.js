...
function calculateAverage(){
  ...
  for(var i = 0; i < $('.column-labels').eq(0).children().length-1; i++){
    ...
    if($('#item'+id+'grade').val() != ""){
      ...
      $('#item'+id+'ptsgained').prop('placeholder',verifGrade.toFixed(3));
    }
    $('#item'+id+'verif').prop('value',verifGrade.toFixed(3));
    sumVerif+= verifGrade.toFixed(3);
  }
...
function showProof(){
    ...
    for(var i = 0; i < $('.column-labels').eq(0).children().length-1; i++){
        ...
        sum += Math.round(10000*parseFloat($('#item'+parseInt(i+1)+'pts').val()))/10000;
    }
